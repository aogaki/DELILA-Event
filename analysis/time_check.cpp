...
{
  for (auto i = 0; i < 17; i++) {
    hist[i] = new TH2D(Form("hist_%d", i), Form("hist_%d", i), 2001, -1000.5,
                       1000.5, 10, -0.5, 9.5);
  }
}
...
void AnalysisThread(TString fileName, uint32_t threadNo)
{
  ...
  for (auto i = 0; i < tree->GetEntries(); i++) {
    ...
    if (ch != -1) {
      for (auto &hit : *hits) {
        if (hit.TimeStamp != 0.) {
          hist[ch]->Fill(hit.TimeStamp, hit.Module);
          hist[16]->Fill(hit.TimeStamp, hit.Module);
        }
      }
    }
  }
...
void time_check(uint32_t runNo = 23)
{
  gSystem->Load("./libEveBuilder.so");
  InitHists();

  auto fileList = GetFileList("./", runNo);
